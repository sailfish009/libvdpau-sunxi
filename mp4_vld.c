...
tab_type *vldTableB16(bitstream *bs, mp4_private_t *priv, int code);
tab_type *vldTableB17(bitstream *bs, mp4_private_t *priv, int code);
 ...
 * 
**/
tab_type tableB16_1[] = // tables to decode Table B16 VLC - 112 values
{ 
...
event_t vld_intra_dct(bitstream *bs, mp4_private_t *priv) 
{
	...
	tab = vldTableB16(bs, priv, show_bits(bs, 12));
	if (!tab) { /* bad code */
		event.run   = 
		event.level = 
		event.last  = -1;
	...
	if (tab->val != ESCAPE) {
		...
		event.level = get_bits(bs, 1) ? -event.level : event.level;
	} else {
		/* this value is escaped - see para 7.4.1.3 */
		/* assuming short_video_header == 0 */
		switch (show_bits(bs, 2)) {
			case 0x0 :  /* Type 1 */
			case 0x1 :  /* Type 1 */
				flush_bits(bs, 1);
				tab = vldTableB16(bs, priv, show_bits(bs, 12));  /* use table B-16 */
				if (!tab) { /* bad code */
					event.run   = 
					event.level = 
					event.last  = -1;
			...
			case 0x3 :  /* Type 3  - fixed length codes */
				flush_bits(bs, 2);
				event.last  = get_bits(bs, 1);
				event.run   = get_bits(bs, 6);  /* table B-18 */ 
				get_bits(bs, 1); /* marker bit */
				event.level = get_bits(bs, 12); /* table B-18 */
				/* sign extend level... */
...
event_t vld_inter_dct(bitstream *bs, mp4_private_t *priv) 
{
	...
	tab = vldTableB17(bs, priv, show_bits(bs, 12));
	if (!tab) { /* bad code */
		event.run   = 
		event.level = 
		event.last  = -1;
	...
	if (tab->val != ESCAPE) {
		...
		/* this value is escaped - see para 7.4.1.3 */
		/* assuming short_video_header == 0 */
		int mode = show_bits(bs, 2);
		switch (mode) {
			case 0x0 :  /* Type 1 */
			case 0x1 :  /* Type 1 */
				flush_bits(bs, 1);
				tab = vldTableB17(bs, priv, show_bits(bs, 12));  /* use table B-17 */
				if (!tab) { /* bad code */
					event.run   = 
					event.level = 
					event.last  = -1;
			...
			case 0x2 :  /* Type 2 */
				flush_bits(bs, 2);
				tab = vldTableB17(bs, priv, show_bits(bs, 12));  /* use table B-16 */
				if (!tab) { /* bad code */
					event.run   = 
					event.level = 
					event.last  = -1;
...
{
	if (intraFlag) {
		return vld_intra_dct(bs, priv);
	} else {
		return vld_inter_dct(bs, priv);
	}
}
...
tab_type *vldTableB16(bitstream *bs, mp4_private_t *priv, int code) {
	...
	if (code >= 512) {
		tab = &(priv->tables.tableB16_1[(code >> 5) - 16]);
	} else if (code >= 128) {
		tab = &(priv->tables.tableB16_2[(code >> 2) - 32]);
	} else if (code >= 8) {
		tab = &(priv->tables.tableB16_3[(code >> 0) - 8]);
	} else {
		/* invalid Huffman code */
		return (tab_type *) NULL;
	}
	flush_bits(bs, tab->len);
	return tab;
}
...
tab_type *vldTableB17(bitstream *bs, mp4_private_t *priv, int code) {
	...
	if (code >= 512) {
		tab = &(priv->tables.tableB17_1[(code >> 5) - 16]);
	} else if (code >= 128) {
		tab = &(priv->tables.tableB17_2[(code >> 2) - 32]);
	} else if (code >= 8) {
		tab = &(priv->tables.tableB17_3[(code >> 0) - 8]);
	} else {
		/* invalid Huffman code */
		return (tab_type *) NULL;
	}
